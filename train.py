...
from model import cc_net, cc_net_new1
from try_vgg16 import vgg16_model
from try_alexnet import Alexnet_model
import csv
...
if __name__ == "__main__":
    ...
    for i in range(len(acc)):
        data_tmp = []
        data_tmp.append(acc[i])
        data_tmp.append(val_acc[i])
        data_tmp.append(loss[i])
        data_tmp.append(val_loss[i])
        result_data.append(data_tmp)

    # 将准确率和损失写入csv中
    with open('result/pdnet_2.csv', 'w', encoding='utf-8', newline='') as file_obj:
        # 创建对象
    ...
    plt.legend()
    plt.savefig("./models_eva/pdnet_train_2.jpg")
    plt.figure()
    ...
    plt.legend()
    plt.savefig("./models_eva/pdnet_loss_2.jpg")
    plt.show()
    # 模型可视化
